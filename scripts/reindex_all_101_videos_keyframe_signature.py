...
def main():
    print("=" * 85)
    print("🚀 RE-INDEXING ALL 101 VIDEOS WITH PEAK-PRESERVING KEYFRAME VISUAL SIGNATURES (50:50)")
    print("=" * 85)
    ...
    with database.snapshot() as snapshot:
        ...
        for row in results:
            ...
            # Find local video path
            local_path = Path("storage/videos") / filename
            if not local_path.exists():
                local_path = Path("storage/videos/distinct_100") / filename
            if not local_path.exists():
                local_path = Path(f"storage/videos/{vid_id}.mp4")
    ...
    print(f"✓ Found {len(all_videos)} indexed videos.")
    
    print("\n2. Extracting FFmpeg Keyframes & Computing Peak-Preserving Visual Signatures in parallel...")
    t_start = time.time()
    ...
    def process_one_video(v):
        try:
            ...
            if np.any(vis_sig != 0):
                fused = 0.50 * vis_sig + 0.50 * txt_emb
    ...
    print(f"✓ Extracted all {len(all_videos)} keyframe visual signatures in {time.time() - t_start:.2f}s!")
    ...
    print("=" * 85)
    print("🎉 SUCCESS: ALL 101 VIDEOS REINDEXED WITH PEAK-PRESERVING KEYFRAME SIGNATURES!")
    print("=" * 85)
